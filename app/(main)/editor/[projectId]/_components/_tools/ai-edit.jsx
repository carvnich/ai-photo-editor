...
import { AlertTriangle, Camera, CheckCircle, Info, Mountain, Sparkles, User, Wand2 } from "lucide-react";
			...
			<div className="bg-amber-500/10 border border-amber-500/20 rounded-lg p-4">
				<div className="flex items-start gap-3">
					...
					<div>
						...
						<p className="text-amber-300/80 text-sm">Please add an image to the canvas first to use AI retouching.</p>
					</div>
				</div>
			</div>
		);
				...
				<div className="bg-green-500/10 border border-green-500/20 rounded-lg p-4">
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="text-green-300/80 text-sm">AI enhancements have been applied to this image</p>
						</div>
					</div>
				</div>
			)}
			...
			<div>
				...
				<div className="grid grid-cols-2 gap-3">
					{RETOUCH_PRESETS.map((preset) => {
						...
						return (
							<div key={preset.key} className={`relative p-4 rounded-lg border cursor-pointer transition-all ${isSelected ? "border-cyan-400 bg-cyan-400/10" : "border-white/20 bg-slate-700/30 hover:border-white/40"}`} onClick={() => setSelectedPreset(preset.key)}>
								<div className="flex flex-col items-center text-center">
									<Icon className="h-8 w-8 text-cyan-400 mb-2" />
									<div className="flex items-center gap-2 mb-1">
										<h4 className="text-white font-medium text-sm">{preset.label}</h4>
										{preset.recommended && <span className="px-1.5 py-0.5 bg-cyan-500 text-white text-xs rounded-full">★</span>}
									</div>
