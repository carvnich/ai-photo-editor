	...
	return (
		<div className="space-y-6">
			...
			<div>
				...
				<p className="text-xs text-white/70 mb-3">Choose one direction to extend your image</p>
				<div className="grid grid-cols-2 gap-3">
					{DIRECTIONS.map(({ key, label, icon: Icon }) => (
						<Button key={key} onClick={() => selectDirection(key)} variant={selectedDirection === key ? "default" : "outline"} className={`flex items-center gap-2 ${selectedDirection === key ? "bg-cyan-500 hover:bg-cyan-600" : ""}`}>
			...
			<div>
				...
				<Slider value={[extensionAmount]} onValueChange={([value]) => setExtensionAmount(value)} min={50} max={500} step={25} className="w-full" disabled={!selectedDirection} />
			</div>
			...
			{selectedDirection && (
				<div className="bg-slate-700/30 rounded-lg p-3">
					...
					<div className="text-xs text-white/70 space-y-1">
						...
						<div className="text-cyan-300">Direction: {DIRECTIONS.find((d) => d.key === selectedDirection)?.label}</div>
					</div>
				</div>
			)}
